...
export class GameController{
  ...
  async craft(item,count=1,depth=0){
    ...
    if(Array.isArray(recipe.ingredients))for(const ing of recipe.ingredients){const per=Math.abs(ing.count||0);if(ing.id!=null&&per)ingredients.push({id:ing.id,count:per*times});}
    for(const ing of ingredients){
      const ingName=this.bot.registry.items[ing.id]?.name;if(!ingName)return false;
      const current=this.bot.inventory.items().filter(i=>i.name===ingName).reduce((n,i)=>n+i.count,0);if(current>=ing.count)continue;
      const missing=ing.count-current,map={oak_planks:"oak_log",spruce_planks:"spruce_log",birch_planks:"birch_log",jungle_planks:"jungle_log",acacia_planks:"acacia_log",dark_oak_planks:"dark_oak_log",mangrove_planks:"mangrove_log",cherry_planks:"cherry_log",stick:"oak_planks",cobblestone:"stone"};
      const source=map[ingName];if(source){const collected=await this.collect(source,Math.min(32,missing));if(!collected&&source.endsWith("_log"))return false;}
      const after=this.bot.inventory.items().filter(i=>i.name===ingName).reduce((n,i)=>n+i.count,0);if(after<ing.count&&!await this.craft(ingName,ing.count-after,depth+1))return false;
    }
    try{
